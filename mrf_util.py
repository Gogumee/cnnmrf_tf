...
def mrf_loss(style_layer, generated_layer, patch_size=3, name=''):
    ...
    with tf.name_scope('mrf_loss' + name):
        ...
        batch, height, width, feature_num = map(lambda i: i.value, generated_layer.get_shape())
        size = height * width * feature_num
        # divide by the size of the image as well as the patch area.
        loss = tf.div(tf.reduce_sum(tf.square(tf.subtract(generated_layer_patches, generated_layer_nn_matched_patches))),
                      size * (patch_size ** 2))
        return loss
